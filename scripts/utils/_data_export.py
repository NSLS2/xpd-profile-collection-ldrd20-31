...
def _read_input_xlsx(fn, sheet_name="inputs", skiprows=1, header=None, index_col=0):
    df = pd.read_excel(
        fn, sheet_name=sheet_name, skiprows=skiprows, header=header, index_col=index_col
    )
    input_dic = {}
    for i in df.index:
        if str(i) == "nan":
...
def _fn_generator(
    uid, beamline_acronym="xpd", stream_name="fluorescence", data_agent="tiled"
):
    _, metadata_dic = read_qepro_by_stream(
        uid,
...
def _exprot_rate_agent(metadata_dic, rate_label_dic, agent_data):
    ...
    for key in rate_label_dic.keys():
        is_key_in_precursors = np.asarray(
            [key in precursor for precursor in metadata_dic["precursors"]]
        )
        ...
        if num_idx.size == 1:
            ...
            try:
                ...
                agent_data[rate_label_dic[key]] = (
                    metadata_dic["infuse_rate"][pre_idx] * ruc * is_infusing[pre_idx]
                )
            except IndexError:
            ...
            raise ValueError(
                f"Check key: {key} of rate_label_dic. More than one precursor was found."
            )
...
def export_qepro_by_stream(
    uid, csv_path, stream_name="primary", data_agent="tiled", plot=False, wait=False
):
    if wait == True:
        time.sleep(2)

    qepro_dic, metadata_dic = read_qepro_by_stream(
        uid, stream_name=stream_name, data_agent=data_agent
    )
    dic_to_csv_for_stream(csv_path, qepro_dic, metadata_dic, stream_name=stream_name)
    print(
        f"Export {stream_name} in uid: {uid[0:8]} to ../{os.path.basename(csv_path)} done!"
    )


def read_qepro_by_stream(
    uid,
    stream_name="primary",
    data_agent="tiled",
    beamline_acronym="xpd-ldrd20-31",
    tiled_client=None,
):
...
def dic_to_csv_for_stream(
    ...
    try:
        ...
        output_mean = fitting[
            "percentile_mean"
        ]  ## add percentile filtering mean data here
    except (TypeError, KeyError):
        if fitting == None:
            pass
        else:
            print(
                "Input fitting info is not correct.\n"
    ...
    if (stream_name == "take_a_uvvis" or stream_name == "primary") and fitting == None:
        ...
        with open(fout, "w") as fp:
            ...
            for i in range(len(pump_names)):
                fp.write(
                    f"{pump_names[i]},{precursor[i]},{infuse_rate[i]},{infuse_rate_unit[i]},{pump_status[i]}\n"
                )
            ...
            for i in range(x_axis_data.shape[1]):
                if spectrum_type == 3:
                    fp.write(
                        f"{x_axis_data[0, i]},{dark_data[0, i]},{reference_data[0, i]},{sample_data[0, i]},{output_data[0, i]}\n"
                    )
                else:
                    fp.write(
                        f"{x_axis_data[0, i]},{dark_data[0, i]},{sample_data[0, i]},{output_data[0, i]}\n"
                    )

    elif (
        stream_name != "take_a_uvvis" and stream_name != "primary"
    ) and fitting == None:
        ...
        for j in range(x_axis_data.shape[0]):
            ...
            with open(fout, "w") as fp:
                ...
                for i in range(x_axis_data.shape[1]):
                    if spectrum_type[0] == 3:
                        fp.write(
                            f"{x_axis_data[j, i]},{dark_data[j, i]},{reference_data[j, i]},{sample_data[j, i]},{output_data[j, i]}\n"
                        )
                    else:
                        fp.write(
                            f"{x_axis_data[j, i]},{dark_data[j, i]},{sample_data[j, i]},{output_data[j, i]}\n"
                        )

    elif type(fitting) is dict:
        if (stream_name == "take_a_uvvis") or (stream_name == "primary"):
            fout = (
                f"{csv_path}/{sample_type}_PL_{date}-{time}_{full_uid[0:8]}_fitted.csv"
            )
        ...
        with open(fout, "w") as fp:
            ...
            try:
                ...
                fp.write(
                    f"plqy,{plqy},PL_integral,{PL_integral},Absorbance_365,{Absorbance_365}\n"
                )
            except (TypeError, KeyError):
                pass
            ...
            for i in range(x_axis_data.shape[1]):
                if spectrum_type[0] == 3:
                    fp.write(
                        f"{x_axis_data[-1, i]},{dark_data[-1, i]},{reference_data[-1, i]},{sample_data[-1, i]},{output_mean[i]},{output_mean[i] - fitted_y[i]}\n"
                    )
                else:
                    fp.write(
                        f"{x_axis_data[-1, i]},{dark_data[-1, i]},{sample_data[-1, i]},{output_mean[i]},{fitted_y[i]}\n"
                    )


def export_qepro_from_uid(
    uid, csv_path, sample_type=None, plot=False, data_agent="db", wait=False
):
    if wait == True:
        time.sleep(2)
...
def dic_to_csv(csv_path, qepro_dic, metadata_dic):
    ...
    with open(fout, "w") as fp:
        ...
        for i in range(len(output_data)):
            if spectrum_type == 3:
                fp.write(
                    f"{x_axis_data[i]},{dark_data[i]},{reference_data[i]},{sample_data[i]},{output_data[i]}\n"
                )
            else:
                fp.write(
                    f"{x_axis_data[i]},{dark_data[i]},{sample_data[i]},{output_data[i]}\n"
                )
...
def ML_to_device_parameters(
    ...
    pump_status = list(
        map(lambda status: status.lower().capitalize() == "Infusing", pump_status)
    )
